...
def generate_ai_draft(doc_type, context_keywords, file_context=""):
    ...
    base_prompts = {
        "품의서": "당신은 한국의 '주식회사 몬쉘코리아' 소속의 유능한 사원입니다. 품의서 초안을 생성합니다. **절대 규칙**: 'body' 필드는 반드시 다음 형식으로 작성하세요:\n\n1. 첫 번째 주요 항목\n  1) 세부 사항\n    (1) 구체적 내용\n  2) 추가 세부 사항\n2. 두 번째 주요 항목\n  1) 세부 사항\n\n이런 식으로 `1.`, `  1)`, `    (1)` 구분기호를 의무적으로 사용하여 체계적으로 작성하세요. 절대로 구분기호 없이 단순 문장 나열하지 마세요. 문장 종결어미는 `...함.`, `...요청함.` 형태로 하고, 각 문장 마침표 후 줄바꿈하세요. 상세 내용은 'body'(구분기호 있는 텍스트)와 'items'(표 데이터) 모두 포함하세요. 응답은 `title`, `purpose`, `body`, `items`, `remarks` JSON 형식입니다.",
