...
st.markdown(f"**'{doc_type}'** 작성을 시작합니다. 아래 양식에 내용을 입력하면 표준 서식의 문서가 자동으로 생성됩니다.")
st.divider()
...
if doc_type == '품의서':
    ...
    if st.button("품의서 생성 및 다운로드", type="primary", use_container_width=True):
        # 합계 자동 계산 로직 추가
        if '단가' in p_data["items_df"].columns and '수량' in p_data["items_df"].columns:
            p_data["items_df"]['합계'] = p_data["items_df"]['단가'] * p_data["items_df"]['수량']
        
        items = p_data["items_df"].to_dict('records')
        total_sum = p_data["items_df"]['합계'].sum() if '합계' in p_data["items_df"].columns else 0

        context = {
            "title": p_data["title"], "purpose": p_data["purpose"].replace('\n', '<br>'),
            "items": items, "total_sum": f"{total_sum:,.0f}",
            "remarks": p_data["remarks"].replace('\n', '<br>'), "generation_date": datetime.now().strftime('%Y-%m-%d')
        }
        template = load_template('pumui_template_v2.html') # 업데이트된 템플릿 사용
...
# ==============================================================================
    ...
    if st.button("공지문 생성 및 다운로드", type="primary", use_container_width=True):
...
# ==============================================================================
    ...
    if st.button("공문 생성 및 다운로드", type="primary", use_container_width=True):
...
# ==============================================================================
    ...
    if st.button("이메일 본문 생성", type="primary", use_container_width=True):
        context = {k: v.replace('\n', '<br>') for k, v in e_data.items()}
        template = load_template('email_template.html')
        html_output = generate_html(template, context)

        st.subheader("📧 이메일 미리보기 및 복사하기")
        st.info("아래 미리보기에서 내용 확인 후, 하단의 'HTML 코드'를 복사하여 이메일에 붙여넣으세요.")
