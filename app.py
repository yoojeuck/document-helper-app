...
def generate_ai_draft(doc_type, context_keywords, file_context=""):
    ...
    prompts = {
        "품의서": {
            ...
            2.  **번호 매기기:** 본문 항목 구분 시 `1.`, `  1)`, `    (1)` 의 위계질서를 준수하는 마크다운을 사용하고, `#` 기호는 사용하지 마세요.
            3.  **출력 형식:** 키워드를 분석하여 'items'(표) 또는 'body'(줄글) 중 하나를 선택하여 `title`, `purpose`, `remarks`와 함께 JSON으로 출력합니다.
            """,
            "user": user_prompt
        },
        "공지문": { "system": "당신은 한국 기업의 사내 커뮤니케이션 담당자입니다. 키워드와 첨부파일 내용을 바탕으로 `1.`, `  1)` 등 마크다운 형식의 번호 매기기를 사용한 '사내 공지문' 초안을 생성합니다. 응답은 'title', 'target', 'summary', 'details', 'contact' key를 포함하는 JSON 형식이어야 합니다.", "user": user_prompt },
...
def renumber_markdown(md_text):
    """마크다운 텍스트의 목록 번호를 계층에 맞게 재정렬하는 함수"""
    lines = md_text.split('\n')
    new_lines = []
    counters = [0, 0, 0] # Level 1, 2, 3
    ...
    for line in lines:
        ...
        if match:
            ...
            content_part = stripped_line[len(match.group(2)):].strip()
            new_lines.append(match.group(1) + new_prefix + content_part)
        else:
...
def clean_text(text):
    ...
    processed_text = renumber_markdown(text)
...
def generate_docx(draft_data, doc_type, signature_data={}):
    ...
    if doc_type == '품의서':
        ...
        doc.add_paragraph()
        doc.add_paragraph(f"{signature_data.get('signature_name', '')} {signature_data.get('signature_title', '')} 드림")
